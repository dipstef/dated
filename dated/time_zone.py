from datetime import timedelta, datetime
from dateutil.tz import tzutc, tzlocal

utc = tzutc()
local = tzlocal()


class LocalOffset(tzlocal):
    def __init__(self, hours):
        super(LocalOffset, self).__init__()
        seconds = timedelta(hours=hours)
        self._dst_offset += seconds
        self._std_offset += seconds


def local_offset(hours):
    return LocalOffset(hours)


def convert_time_zone(date_time, from_tz, to_tz):
    return date_time.replace(tzinfo=from_tz).astimezone(to_tz)


def convert_to_local_offset(date_time, plus):
    return convert_time_zone(date_time, from_tz=local, to_tz=LocalOffset(hours=plus))


def local_utc_offset():
    return local.utcoffset(datetime.now())


class TimeZoneDateTime(object):

    def __init__(self, timezone):
        self._timezone = timezone

    def to_time_zone(self, date_time, timezone):
        return convert_time_zone(date_time, from_tz=self._timezone, to_tz=timezone)